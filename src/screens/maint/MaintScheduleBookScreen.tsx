...
export function MaintScheduleBookScreen() {
  ...
  // Services were chosen on "What service do you need?" → this screen leads with date & time.
  const pickMode = useAppStore((s) => s.serviceTypePick.length > 0);
  ...
  return (
    <Screen>
      ...
      <View style={{ marginBottom: spacing.sm }}>
          ...
          {/* Chosen services — compact summary; "Change" reveals the full list. */}
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.section, marginBottom: spacing.md }}>
            ...
            <Tappable onPress={() => setEditingServices((v) => !v)} hitSlop={8}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: colors.primaryDark }}>{editingServices ? 'Done' : 'Change'}</Text>
            </Tappable>
          </View>
          {editingServices ? (
            <>
