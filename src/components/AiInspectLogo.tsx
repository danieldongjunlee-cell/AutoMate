...
/**
 * "AI Repair Estimate" hero icon: the AI magnifying glass inspecting a car.
 * Metallic gradients + rim lights sell the 3D read at icon size. Renders
 * fully opaque so it reads as an icon.
 */
export function AiInspectLogo({ size = 120, opacity = 1 }: { size?: number; opacity?: number }) {
  ...
  return (
    <Svg width={size} height={height} viewBox="0 0 120 88" opacity={opacity}>
      ...
      {/* Magnifying glass hovering over the car's front fender. */}
      {/* Handle: angled down-left from the lens rim. */}
      <Path d="M29.5 42 L15 58.5" stroke="#23314d" strokeWidth={8} strokeLinecap="round" />
      <Path d="M29.5 42 L15 58.5" stroke="#4a5f86" strokeWidth={3.5} strokeLinecap="round" />
      <Circle cx="41" cy="30" r="16" fill="url(#ai-lensglass)" stroke="url(#ai-lensrim)" strokeWidth={5.5} />
      <Circle cx="41" cy="30" r="13.4" fill="none" stroke="#eaf2ff" strokeWidth={1} opacity={0.7} />
      {/* Glass glare */}
      <Path d="M33 23.5 Q38 19.5 45.5 21" stroke="#ffffff" strokeWidth={2.2} strokeLinecap="round" opacity={0.85} />
      {/* AI sparkle inside the lens (it's "thinking"/analyzing). */}
      <Path d={sparkle(41, 30, 8)} fill="#2e6bff" />
      <Path d={sparkle(41, 30, 3.8)} fill="#ffffff" />
